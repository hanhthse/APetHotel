...
public class addFeedBackController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String feedback = request.getParameter("customerFeedback");
            feedbackDAO feedDAO = new feedbackDAO();
            int feedbackList = feedDAO.addFeedBack(bookingId, rating, feedback);
            // Xử lý dữ liệu feedback ở đây (lưu trữ, phản hồi, v.v.)
            request.setAttribute("feedDAO", feedDAO);
            request.setAttribute("feedbackList", feedbackList);
            // Chuyển hướng người dùng sau khi xử lý
            request.getRequestDispatcher("mainControllers?action=feedback").forward(request, response);
        }
    }
